...
dt.df.sig <- dt.df[which(dt.df$P.adjusted <0.15),]
...
```{r,echo=FALSE}
promising.experiments <- c(levels.experiment[18],levels.experiment[6],levels.experiment[9],levels.experiment[19])
stochastic.order <- dft.t$Freq
pander(data.frame(experiment=promising.experiments,stochastic.order=stochastic.order), caption = "Most promising experiments")
```
